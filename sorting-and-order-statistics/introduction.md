...
the keys, it must permute the _satellite data_ as well. If each record includes a large
...
* Algorithms often use sorting as a key subroutine. For example, a program that
  ...
  objects from bottom to top. We shall see numerous algorithms that use sorting
  as a subroutine.
...
* Many engineering issues come to the fore when implementing sorting algorithms.
  ...
  such as prior knowledge about the keys and satellite data, the memory
  hierarchy (caches and virtual memory) of the host computer, and the
...
a sorting algorithm sorts _in place_ if only a constant number of elements of the input
...
Heap sort sorts n numbers in place in O(n*lg*n) time. It uses an important data structure,
...
Its expected running time is Θ(n*lg*n), however, and it generally outperforms heapsort
...
sorts. Using this model, we prove a lower bound of Ω(n*lg*n) on the worst-case running
...
We then goes on to show that we can beat this lower bound of Ω(n*lg*n)
...
|  Merge sort |  O(n*lg*n) |  Θ(n*lg*n) |
| Heap sort  |  O(n*lg*n) |  - |
| Quick sort  |  O(n^2) | Θ(n*lg*n) (expected)  |
...
We can, of course, select the *i*th order statistic by sorting the input and indexing
the *i*th element of the output. With no assumptions about the input distribution,
this method runs in Ω(n*lg*n) time, as the lower bound proved in the decision-tree model shows.
