...
分析算法的结果意味着预测算法需要的资源。虽然有时我们主要关心像内存、通信带宽或计算机硬件这类资源，但是通常我们想度量的是计算时间。一般来说，通过分析求解某个问题的集中候选算法，我们可以选出一种最有效的算法。这种额分析可能指出不止一个可行的候选算法，但是在这个过程中，我们往往可以抛弃几个较差的算法。

在能够分析一个算法之前，我们必须有一个要是用的实现技术的模型，包括描述所用资源及其代价的模型。我们假定一种通用的单处理器计算模型-随机访问机(random-access machine, RAM)来作为我们的实现技术，算法可以用计算机程序来实现。在RAM模型中，指令一条接一条地执行，并没有并发操作。
